...
class MultigridLevel_TR2D(MultigridLevel_Base):
  ...
  def generate_interp(self):
    ...
    if self.mg_opts.sparse:
      ...
      self.restrictmat = sp.lil_matrix((n, n_coarse))
    else:
      self.restrictmat = np.zeros((n, n_coarse))

    for i in range(n):
      coord = self.index_to_ijk(i)
      coarse_indices = self.get_coarse_neighbors(coord, not i%2)
      if i%2:
        self.restrictmat[i, coarse_indices] = 1./(2*self.num_dim)
      else:
        self.restrictmat[i, coarse_indices] = 1.

    if self.mg_opts.sparse:
      self.restrictmat = self.restrictmat.transpose()/2
      self.restrictmat = sp.csr_matrix(self.restrictmat)
      self.restrictmat.maxprint = 500
      print self.restrictmat
    exit()
    ...
    self.interpmat.maxprint = 500
    print self.interpmat
